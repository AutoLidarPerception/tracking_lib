...
#ifndef HM_TRACKING_WORKER_HPP_
#define HM_TRACKING_WORKER_HPP_

#include "base_tracking_worker.h"
#include "filters/base_filter.hpp"
#include "trackers/multi_object_tracker.hpp"
#include "matchers/base_tracker_obsv_matcher.h"
...
namespace tracking {
    ...
    public:
        HmTrackingWorker(TrackingWorkerParams params);
        ...
        virtual inline const std::map<IdType, Trajectory>& collectTrajectories() const
        {
            ...
            for( ; iter_period != trajectory_periods_.end(); ) {
                if (iter_period->second > 0.) {
                    ...
                    if (iter_ids != trajectory_segments_.end()) {
                        fixedTrajectories.push_back(std::make_tuple(iter_period->second, iter_ids->second));
        ...
        //TODO just collect fixed trajectory's tracker id and period
        ...
        /**
         * @brief Get predicted objects for over-/under-segmentation classify with current segments
         * @return
         */
                                                              ...
                                                              const Eigen::Matrix4d& trans_pose) const
        {
            ...
            if (booted_ && ((time_diff - 0.) > common::EPSILON)) {
                ...
                for (size_t i = 0u; i < trackers.size(); ++i) {
                    ...
                    // position states restore original world coordinates 局部坐标-->世界坐标
                    //obj->ground_center = result_obj->ground_center.cast<double>() - global_to_local_offset_;
                    ...
                    // restore original local Velodyne coordinates 世界坐标-->局部坐标
                    common::transform::transformPointCloud<PointI>(pose_world2velo, obj->cloud);
                    //TODO restore convex hull of the object
