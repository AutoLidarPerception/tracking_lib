...
namespace tracking {
    ...
    HmTrackingWorker::HmTrackingWorker(TrackingWorkerParams params)
            : params_(params),
              matcher_(nullptr),
              time_stamp_(0.0),
              booted_(false)
    {
        ...
        if (filter_method_ == ROBUST_KALMAN_FILTER) {
            ...
            /*
              ...
              */
            if (!RobustKalmanFilter::initNoiseParams(params_.filter_measurement_noise,
        ...
        if (!setCollectConsecutiveInvisibleMaximum(params_.tracking_collect_consecutive_invisible_maximum)) {
            ROS_ERROR_STREAM("Failed to set collect consecutive invisible maximum! " << name());
            return;
        }*/
        // feature extractor
        feature_extractor_ = new feature::HistogramFeatureExtractor();
        feature_extractor_->setHistogramBinSize(params_.tracking_histogram_bin_size);
     ...
     * @params[OUT] objects_tracked: tracked objects with tracking information 当前帧处理后的追踪障碍物信息
                                 ...
                                 std::vector<ObjectPtr>* objects_tracked)
    {
        ...
        ///@note 追踪状态位于起始位置全局坐标系(World)/物体检测位于 Velodyne 局部坐标系
        ...
        constructTrackedObjects(objects_obsved, &objects_trackable_transformed, velo2world_pose_, options);
        ...
        matcher_->match(&objects_trackable_transformed, trackers, tracker_predicts,
                        &assignments,
                        &unassigned_trackers, &unassigned_obsvs);
        ...
        //TODO add new obsvs to assigned tracker's trajectory
        ...
        /*std::vector<ObjectTrackerPtr> lost_trackers = multi_object_tracker_->getLostTrackers();
        for (size_t idx = 0u; idx < lost_trackers.size(); ++idx) {
            ...
            if (iter != previous_poses_.end()) {
                ...
                auto iter_periods = trajectory_periods_.find(lost_trackers[idx]->idx_);
        ...
        //multi_object_tracker_->createNewTrackers(objects_trackable_transformed, unassigned_obsvs);
        multi_object_tracker_->createNewTrackers(objects_trackable_transformed, unassigned_obsvs,
                                                 &trajectory_segments_, &trajectory_periods_);
                                      ...
                                      std::vector<ObjectPtr>* objects_tracked)
    {
        ...
        /*
         * TODO 追踪状态位于起始位置全局坐标系(World)/物体检测位于 Velodyne 局部坐标系
         *  观测量需要投影到世界坐标系, 统一在该坐标系下进行追踪状态更新
         *  返回可视化(可能还有其他用途...)的追踪物体则需要投影回 Velodyne 局部坐标系
         */
        ...
        /*TransformPoseGlobal2Local(&velo2world_pose);
        ADEBUG << "velo2local_pose\n" << velo2world_pose;*/
        // B.2 construct tracked objects(bounding box+states[anchor_point;velocity;acceleration;])
        std::vector<TrackableObjectPtr> objects_trackable_transformed;
        // 从当前帧检测到的障碍物初始化追踪信息
        ...
        // C. create trackers
        // 初次追踪时将上一步构造的所有追踪障碍物transformed_objects绑定追踪器, 记录在系统追踪器列表object_tracks_中
        std::vector<int> unassigned_obsvs;
        ...
        //TODO D. collect system tracked results 筛选可靠的追踪障碍物输出
        collectTrackingObjects(objects_tracked);
     ...
     * @brief construct tracked objects via necessray transformation & feature computing
                                                   ...
                                                   const TrackingOptions& options)
    {
        ...
        for (size_t i = 0u; i < num_obsvs; ++i) {
            ...
            // Computing shape featrue 计算每个障碍物的形状特征: 点云直方图 ==> calculate match distance
     ...
     * @note transform from current velodyne pose into world coordinate(localization)
     * @param object_tracked
     * @param pose
     */
                                                  ...
                                                  const Eigen::Matrix4d& pose)
    {
        ...
        ///@note Trackable Objects inherit direction/ground center from no-transformed built objects,
        ///     compute barycenter from no-transformed cloud
        // transform direction
    ...
    void HmTrackingWorker::collectTrackingObjects(std::vector<ObjectPtr>* objects_tracked)
    {
        ...
        for (size_t i = 0u; i < trackers.size(); ++i) {
            ...
            //TODO maintain collected objects' trajectory, extend or create object's trajectory
            ...
            //TODO collect objects in World coordinate
