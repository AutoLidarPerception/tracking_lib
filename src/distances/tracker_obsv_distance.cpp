...
namespace tracking {
    ...
    bool TrackerObsvDistance::setLocationDistanceWeight(const float& location_distance_weight)
    {
        if (location_distance_weight >= 0) {
            s_location_distance_weight_ = location_distance_weight;
            ROS_INFO("location distance weight of %s is %f.", Name().c_str(), s_location_distance_weight_);
            return true;
        }
    ...
    bool TrackerObsvDistance::setDirectionDistanceWeight(const float& direction_distance_weight)
    {
        if (direction_distance_weight >= 0) {
            s_direction_distance_weight_ = direction_distance_weight;
            ROS_INFO("direction distance weight of %s is %f.", Name().c_str(), s_direction_distance_weight_);
            return true;
        }
    ...
    bool TrackerObsvDistance::setBboxSizeDistanceWeight(const float& bbox_size_distance_weight)
    {
        if (bbox_size_distance_weight >= 0) {
            s_bbox_size_distance_weight_ = bbox_size_distance_weight;
            ROS_INFO("bbox size distance weight of %s is %f.", Name().c_str(), s_bbox_size_distance_weight_);
            return true;
        }
    ...
    bool TrackerObsvDistance::setPointNumDistanceWeight(const float& point_num_distance_weight)
    {
        if (point_num_distance_weight >= 0) {
            s_point_num_distance_weight_ = point_num_distance_weight;
            ROS_INFO("point num distance weight of %s is %f.", Name().c_str(), s_point_num_distance_weight_);
            return true;
        }
    ...
    bool TrackerObsvDistance::setHistogramDistanceWeight(const float& histogram_distance_weight)
    {
        if (histogram_distance_weight >= 0) {
            s_histogram_distance_weight_ = histogram_distance_weight;
            ROS_INFO("histogram distance weight of %s is %f.", Name().c_str(), s_histogram_distance_weight_);
            return true;
        }
    ...
    /**
     * @brief 根据一系列关联特征(包括运动一致性, 外观一致性等)计算追踪器到观测可追踪障碍物之间的距离
     ...
     * @return
     */
    float TrackerObsvDistance::computeDistance(ObjectTrackerConstPtr tracker,
                                               ...
                                               TrackableObjectConstPtr object_obsved)
    {
        ...
        float location_dist = computeLocationDistance(tracker, tracker_predict, object_obsved);
    ...
    //TODO 这个需要一些理论依据寻找
    float TrackerObsvDistance::computeLocationDistance(ObjectTrackerConstPtr tracker,
                                                       const Eigen::VectorXf& tracker_predict,
                                                       TrackableObjectConstPtr object_obsved)
    {
        ...
        /* Assume location distance is generated from a normal distribution with 具有对称方差的正态分布
    ...
    //TODO
    float TrackerObsvDistance::computeDirectionDistance(ObjectTrackerConstPtr tracker,
                                                        const Eigen::VectorXf& tracker_predict,
                                                        TrackableObjectConstPtr object_obsved)
    {
        ...
        if (!anchor_point_shift.head(2).isZero() &&
            ...
            cos_theta = common::geometry::computeCosTheta2dXyBetweenVectors(predicted_tracker_motion,
                                                                            anchor_point_shift);
        }
    ...
    //TODO 需要结合边框构建进行思考
    float TrackerObsvDistance::computeBboxSizeDistance(ObjectTrackerConstPtr tracker,
                                                       TrackableObjectConstPtr object_obsved)
    {
    ...
    // 点云数目距离
    float TrackerObsvDistance::computePointNumDistance(ObjectTrackerConstPtr tracker,
                                                       TrackableObjectConstPtr object_obsved)
    {
    ...
    /**
     * 点云直方图特征L1距离∑|old_object_shape_features[0...N] - new_object_shape_features[0...N]|;
     * TODO How to compute TrackedObject's shape_features
     *  computed by FeatureDescriptor==>(3*bin_size) feature vector
     */
    float TrackerObsvDistance::computeHistogramDistance(ObjectTrackerConstPtr tracker,
                                                        TrackableObjectConstPtr object_obsved)
    {
        // Compute histogram distance for given tracker & obsved object
        // range from 0 to 3
        TrackableObjectConstPtr last_obsv = tracker->current_object_;
        const Feature& old_object_shape_features = last_obsv->object_ptr->shape_features;
        const Feature& new_object_shape_features = object_obsved->object_ptr->shape_features;
        if (old_object_shape_features.size() != new_object_shape_features.size()) {
            ROS_ERROR("sizes of compared features not matched! ObjectTrackerDistance");
            return FLT_MAX;
        }

        float histogram_distance = 0.0;
        for (size_t i = 0u; i < old_object_shape_features.size(); ++i) {
            histogram_distance += std::fabs(old_object_shape_features.at(i).value - new_object_shape_features.at(i).value);
        }
