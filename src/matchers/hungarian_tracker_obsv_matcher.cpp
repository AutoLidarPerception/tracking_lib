...
namespace tracking {
    ...
    bool HungarianTrackerObsvMatcher::setMatchDistanceMaximum(const float& match_distance_maximum)
    {
     ...
     *  update tracker-associate observed object's associate score(->association_score)
     *  find unassigned trackers/observed objects
     */
    void HungarianTrackerObsvMatcher::match(std::vector<TrackableObjectPtr>* objects_obsved,
    ...
    {
        // A. computing association matrix 计算关联距离矩阵 tracks.size()*objects->size()
        Eigen::MatrixXf association_mat(trackers.size(), objects_obsved->size());
        // 需要结合追踪器六维度的预测状态 ==> 计算关联距离矩阵(association_mat)
        computeAssociateMatrix(trackers, trackers_predict, *objects_obsved, &association_mat);
        ...
        for (size_t i = 0u; i < tracker_components.size(); ++i) {
            ...
            matchInComponent(association_mat,
                             tracker_components[i], obsv_components[i],
                             &sub_assignments,
     ...
     */
    void HungarianTrackerObsvMatcher::computeAssociateMatrix(const std::vector<ObjectTrackerPtr>& trackers,
                                                             const std::vector<Eigen::VectorXf>& trackers_predict,
                                                             const std::vector<TrackableObjectPtr>& objects_obsved,
                                                             Eigen::MatrixXf* association_mat)
    {
        // Compute matrix of association distance
        // 	每个元素[rowIdx][colIdx]是追踪器tracks[rowIdx]跟检测到的可追踪障碍物new_objects[colIdx] 之间的综合考量距离
        for (size_t i = 0u; i < trackers.size(); ++i) {
            for (size_t j = 0u; j < objects_obsved.size(); ++j) {
                ...
                (*association_mat)(i, j) = TrackerObsvDistance::computeDistance(trackers[i], trackers_predict[i],
                                                                                objects_obsved[j]);
            }
        }
    }
     ...
     * @return nothing
     */
    void HungarianTrackerObsvMatcher::computeConnectedComponents(const Eigen::MatrixXf& association_mat,
     ...
     *  单个连通分量内的追踪器-观测匹配
     * @params[IN] association_mat: association matrix of all objects to tracks 关联距离矩阵
     * @params[IN] tracker_component: component of track 本连通分量内的追踪(通过index索引)
     * @params[IN] objects_obsved_component: component of object 本连通分量内的检测(通过index索引)
     * @params[OUT] sub_assignments: component assignment pair of object & track 本连通分量内的跟踪-检测最佳匹配
     * @params[OUT] sub_unassigned_trackers: component tracks not matched 本连通分量内未能匹配检测的追踪(通过index索引)
     * @params[OUT] sub_unassigned_objects_obsved: component objects not matched 本连通分量内未能匹配追踪的检测(通过index索引)
     * @return nothing
     */
    void HungarianTrackerObsvMatcher::matchInComponent(const Eigen::MatrixXf& association_mat,
                                                       ...
                                                       std::vector<int>* sub_unassigned_objects_obsved)
    {
        ...
        if (tracker_component.empty()) {
            for (size_t i = 0u; i < objects_obsved_component.size(); ++i) {
                sub_unassigned_objects_obsved->push_back(objects_obsved_component[i]);
            }
        }
        ...
        if (tracker_component.size() == 1
            && objects_obsved_component.size() == 1) {
        ...
        assignObsvedObjects2Trackers(local_association_mat, s_match_distance_maximum_,
                                     &local_assignments,
                                     &local_unassigned_trackers,
                                     &local_unassigned_obsvs);

        for (size_t i = 0u; i < local_assignments.size(); ++i) {
            int global_tracker_id = local2global_tracker[local_assignments[i].first];
        ...
        for (size_t i = 0u; i < local_unassigned_trackers.size(); ++i) {
            int global_track_id = local2global_tracker[local_unassigned_trackers[i]];
     ...
     * @params[IN] association_mat: matrix of association distance 本连通分量内的关联距离矩阵
     ...
     * @return nothing
     */
    void HungarianTrackerObsvMatcher::assignObsvedObjects2Trackers(const Eigen::MatrixXf& association_mat,
                                                                     ...
                                                                     std::vector<int>* unassigned_objects_obsved)
    {
        ...
        ///@note build cost 构建本连通分量内的代价图(追踪器->观测的代价+观测->观测的代价)
        ...
        for (size_t i = 0u; i < tracker_idxs.size(); ++i) {
            ...
            //Minimize-cost matches should satisfy threshold(通过关联距离阈值筛选最佳匹配)
            if (association_mat(tracker_idxs[i], obsv_idxs[i]) < assign_distance_maximum) {
                (*assignments)[num_assignments++] = std::make_pair(tracker_idxs[i], obsv_idxs[i]);
