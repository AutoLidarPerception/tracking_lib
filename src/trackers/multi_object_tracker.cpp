...
namespace tracking {
    ...
    //----------------------------------------- init -----------------------------------------//
    bool MultiObjectTracker::setTrackerConsecutiveInvisibleMaximum(const int& track_consecutive_invisible_maximum)
    {
        if (track_consecutive_invisible_maximum >= 0) {
            s_tracker_consecutive_invisible_maximum_ = track_consecutive_invisible_maximum;
    ...
    bool MultiObjectTracker::setTrackerVisibleRatioMinimum(const float& track_visible_ratio_minimum)
    {
     ...
     * @return nothing
     */
    void MultiObjectTracker::execPredict(std::vector<Eigen::VectorXf>* tracker_predicts,
     ...
     * @param time_diff
     */
    void MultiObjectTracker::updateAssignedTrackers(std::vector<TrackableObjectPtr>* obsvs_trackable,
                                                    ...
                                                    const double& time_diff)
    {
        for (size_t i = 0u; i < assignments.size(); ++i) {
            ...
            trackers_[tracker_id]->updateWithObservation((*obsvs_trackable)[obsv_id], time_diff);
        }
    }
     ...
     */
    void MultiObjectTracker::updateUnassignedTrackers(const std::vector<Eigen::VectorXf>& tracker_predicts,
                                                      ...
                                                      const double& time_diff)
    {
        ...
        for (size_t i = 0u; i < trackers_unassigned.size(); ++i) {
            ...
            trackers_[tracker_idx]->updateWithoutObservation(tracker_predicts[tracker_idx], time_diff);
     ...
     */
    void MultiObjectTracker::createNewTrackers(const std::vector<TrackableObjectPtr>& obsvs_trackable,
                                               const std::vector<int>& unassigned_ids)
    {
        ...
        for (size_t i = 0u; i < unassigned_ids.size(); ++i) {
            ...
            // 为每一个追踪障碍物绑定障碍物追踪器
            ObjectTrackerPtr tracker(new ObjectTracker(obsvs_trackable[obsv_id], getNextTrackerId()));
            trackers_.push_back(tracker);
        }
        ROS_INFO_STREAM("HmTrackingWorker::created " << unassigned_ids.size() << " new Trackers. Took "
    ...
    int MultiObjectTracker::removeLostTrackers()
    {
        ...
        for (size_t i = 0u; i < trackers_.size(); ++i) {
            ...
            float tracker_visible_ratio = trackers_[i]->total_visible_count_ * 1.0f / trackers_[i]->age_;
            ...
            int tracker_consecutive_invisible_count = trackers_[i]->consecutive_invisible_count_;
            if (tracker_consecutive_invisible_count > s_tracker_consecutive_invisible_maximum_) {
                lost_trackers_.push_back(trackers_[i]->idx_);
                continue;
            }
            ...
            if (i == valid_tracker_num) {
                ...
                // move current valid tracker #i forward, lost tracker #valid_tracker_num backward
    ...
    int MultiObjectTracker::removeLostTrackers(std::map<IdType, Trajectory>* trajectory_poses,
                                               std::map<IdType, double>* trajectory_periods)
    {
        size_t valid_tracker_num = 0;
        lost_trackers_.clear();
        for (size_t i = 0u; i < trackers_.size(); ++i) {
            ...
            if (tracker_visible_ratio < s_tracker_visible_ratio_minimum_){
                ...
                if (iter != (*trajectory_poses).end()) {
                    ...
                    auto iter_periods = (*trajectory_periods).find(trackers_[i]->idx_);
