...
namespace tracking {
     ...
     * @param obj
     * @note Apollo's doc
     *  obj: 经过CNN检测-->Min_Box包裹-->ConstructTrackedObjects 构造的当前帧的一个可追踪障碍物
     */
    ObjectTracker::ObjectTracker(TrackableObjectPtr obj, IdType tracker_id)
    {
        ...
        if (s_filter_method_ == ROBUST_KALMAN_FILTER) {
            ...
            filter_ = new RobustKalmanFilter();
            ROS_WARN("invalid filter method! default filter (RobustKalmanFilter) in use!");
        }
        ...
        //TODO Initialize object direction with its lane direction
    ...
    bool ObjectTracker::setTrackerCachedHistorySizeMaximum(const int& tracker_cached_history_size_maximum)
    {
        if (tracker_cached_history_size_maximum > 0) {
            s_tracker_cached_history_size_maximum_
                    = tracker_cached_history_size_maximum;
            ROS_INFO_STREAM("tracker cached history size maximum of object tracker is "
                                    << s_tracker_cached_history_size_maximum_);
            return true;
        }
    ...
    bool ObjectTracker::setAccelerationNoiseMaximum(const double& acceleration_noise_maximum)
    {
    ...
    Eigen::VectorXf ObjectTracker::execPredict(const double& time_diff)
    {
        ...
        // 返回六维状态向量: 箭头锚点(belief_anchor_point_(x, y, z)); 箭头方向大小(belief_velocity_(x, y, z))
        // 基于CVM的状态预测 + KF更新: 预测协方差矩阵 [KF预测]
        Eigen::VectorXf filter_predict = filter_->execPredict(time_diff);

        //TODO filter_->execPredict()里面也进行了相似的更新
        // Get the predict of track (update tracked object when you update the state of track !!!!!) [追踪器预测]
        Eigen::VectorXf tracker_predict = filter_predict;
        tracker_predict(0) = belief_anchor_point_(0) + belief_velocity_(0) * time_diff;
        tracker_predict(1) = belief_anchor_point_(1) + belief_velocity_(1) * time_diff;
        tracker_predict(2) = belief_anchor_point_(2) + belief_velocity_(2) * time_diff;
    ...
    Eigen::VectorXf ObjectTracker::getExpectedState(const double& time_diff)
    {
        ...
        expected_state(0) = belief_anchor_point_(0) + belief_velocity_(0) * time_diff;
        expected_state(1) = belief_anchor_point_(1) + belief_velocity_(1) * time_diff;
        expected_state(2) = belief_anchor_point_(2) + belief_velocity_(2) * time_diff;
                                              ...
                                              const double& time_diff)
    {
        ...
        belief_velocity_accelaration_ = (object_obsved->velocity - current_object_->velocity) / time_diff;
    ...
    /**
     * @brief update object tracker without observation but predict velocity in "predict_state"
     * @param predict_state
     * @param time_diff
     */
    void ObjectTracker::updateWithoutObservation(const Eigen::VectorXf& predict_state,
                                                 const double& time_diff)
    {
    ...
    /**
     * @brief
     *  smooth tracker orientation as obvious velocity's direction or observation's direction
    ...
    void ObjectTracker::smoothTrackerOrientation()
    {
        ...
        if (velocity_is_obvious) {
            ...
            //TODO lane_direction needs HD Map
        ...
        common::bbox::computeBboxSizeCenter<PointICloudPtr>(current_object_->object_ptr->cloud,
                                                            current_dir.cast<double>(),
                                                            &new_size, &new_center);
     ...
     * @note by checking previous/current consecutive velocity's angle change is greater than 45 degree
     * @param time_diff
    ...
    bool ObjectTracker::checkTrackerStaticHypothesis(const double& time_diff)
    {
        ...
        bool is_velocity_angle_change = checkTrackerStaticHypothesisByVelocityAngleChange(time_diff);
     ...
     * @return
     */
    bool ObjectTracker::checkTrackerStaticHypothesisByVelocityAngleChange(const double& time_diff)
    {
        ...
        double velocity_angle_change =
                common::geometry::computeTheta2dXyBetweenVectors<Eigen::Vector3f>(previous_velocity,
                                                                                  current_velocity);
