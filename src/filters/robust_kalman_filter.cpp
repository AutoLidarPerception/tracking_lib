...
namespace tracking {
     ...
     * | 0  0   10|
     */
    Eigen::Matrix3d RobustKalmanFilter::s_propagation_noise_ = 10 * Eigen::Matrix3d::Identity();
    ...
    void RobustKalmanFilter::setUseAdaptive(const bool& use_adaptive)
    {
        ...
        ROS_INFO_STREAM("use adaptive of RobustKalmanFilter is " << s_use_adaptive_);
    }

    bool RobustKalmanFilter::setAssociationScoreMaximum(const double& association_score_maximum)
    {
    ...
    bool RobustKalmanFilter::setBreakdownThresholdMaximum(const double& breakdown_threshold_maximum)
    {
    ...
    // @brief initialize the state of filter with trackable object's anchor point and velocity 速度初始化为零
    ...
    ///@retval 返回六维状态向量: 箭头锚点(belief_anchor_point_(x, y, z)); 箭头方向大小(belief_velocity_(x, y, z))
     ...
     * @param time_diff
     */
    void RobustKalmanFilter::updateWithObservation(TrackableObjectConstPtr new_object,
                                                   TrackableObjectConstPtr old_object,
                                                   const double& time_diff)
    {
        if (time_diff <= DBL_EPSILON) {
            ROS_WARN("[RobustKalmanFilter] Time diff is too limited to updating Robust Kalman Filter!");
            return;
        }
        ...
        //velocity
        Eigen::Vector3f measured_velocity = calcMeasuredVelocity(new_object, old_object, time_diff);
        //accleration
        Eigen::Vector3f measured_acceleration = calcMeasuredAcceleration(measured_velocity, time_diff);
    ...
    /**
     * @brief update "update_quality_" for adaptive filtering 评估卡尔曼修正质量:[0,1]
                                               ...
                                               TrackableObjectConstPtr old_object)
    {
        ...
        float update_quality_according_association_score = calcUpdateQualityAccordingAssociationScore(new_object);
        // Strategy B: according to point number change
        float update_quality_according_point_num_change = calcUpdateQualityAccordingPointNumChange(new_object, old_object);
    ...
    float RobustKalmanFilter::calcUpdateQualityAccordingAssociationScore(TrackableObjectConstPtr new_object)
    {
    ...
    float RobustKalmanFilter::calcUpdateQualityAccordingPointNumChange(TrackableObjectConstPtr new_object,
                                                                       TrackableObjectConstPtr old_object)
    {
        ...
        float update_quality = 1 - fabs(new_pt_num - old_pt_num) / std::max(new_pt_num, old_pt_num);
        return update_quality;
    }
    //--------------------------------- Adaptive KF update quality ---------------------------------//
     ...
     */
    Eigen::VectorXf RobustKalmanFilter::calcMeasuredVelocity(TrackableObjectConstPtr new_object,
                                                             TrackableObjectConstPtr old_object,
                                                             const double& time_diff)
    {
        // Observation I: anchor point velocity measurment
        Eigen::Vector3f measured_anchor_point_velocity = calcMeasuredAnchorPointVelocity(new_object, old_object, time_diff);
        // Observation II: bbox center velocity measurment
        Eigen::Vector3f measured_bbox_center_velocity = calcMeasuredBboxCenterVelocity(new_object, old_object, time_diff);
        // Observation III: bbox corner velocity measurment
        Eigen::Vector3f measured_bbox_corner_velocity = calcMeasuredBboxCornerVelocity(new_object, old_object, time_diff);
    ...
    Eigen::VectorXf RobustKalmanFilter::calcMeasuredAnchorPointVelocity(TrackableObjectConstPtr new_object,
                                                                        TrackableObjectConstPtr old_object,
                                                                        const double& time_diff)
    {
        Eigen::Vector3f measured_anchor_point_velocity = new_object->anchor_point - old_object->anchor_point;
        measured_anchor_point_velocity /= time_diff;
        measured_anchor_point_velocity(2) = 0.0;
        return measured_anchor_point_velocity;
    }
    //TODO Compute 2D bbox center velocity measurment 边框中心估计速度
    ///@note predict object size and ground center based on object cloud and previous direction
    Eigen::VectorXf RobustKalmanFilter::calcMeasuredBboxCenterVelocity(TrackableObjectConstPtr new_object,
                                                                       TrackableObjectConstPtr old_object,
                                                                       const double& time_diff)
    {
        ...
        //predict object size and ground center based on object cloud and previous direction
        Eigen::Vector3d new_size = old_size;
        Eigen::Vector3d new_center = old_center;
        common::bbox::computeBboxSizeCenter<PointICloudPtr>(new_object->object_ptr->cloud,
                                                            old_dir,
                                                            &new_size, &new_center);
    ...
    //TODO Compute 2D bbox corner velocity measurment 边框顶点估计速度
    Eigen::VectorXf RobustKalmanFilter::calcMeasuredBboxCornerVelocity(TrackableObjectConstPtr new_object,
                                                                       TrackableObjectConstPtr old_object,
                                                                       const double& time_diff)
    {
        Eigen::Vector3f project_dir = new_object->anchor_point - old_object->anchor_point;
        ...
        float min_bbox_corner_velocity_on_project_dir_gain_norm = min_bbox_corner_velocity_on_project_dir.norm();
        for (size_t i = 0u; i < 4; ++i) {
            old_bbox_corner = old_bbox_corner_list[i];
            new_bbox_corner = new_bbox_corner_list[i];
            Eigen::Vector3f bbox_corner_velocity = ((new_bbox_corner - old_bbox_corner) / time_diff).cast<float>();
            ...
            if (bbox_corner_velocity_on_project_dir_gain.norm() < min_bbox_corner_velocity_on_project_dir_gain_norm) {
                min_bbox_corner_velocity_on_project_dir = bbox_corner_velocity_on_project_dir;
                min_bbox_corner_velocity_on_project_dir_gain_norm = bbox_corner_velocity_on_project_dir_gain.norm();
            }
        }
    ...
    Eigen::Vector3f RobustKalmanFilter::selectMeasuredVelocity(const std::vector<Eigen::Vector3f>& candidates)
    {
        // Strategy I: accoridng motion consistency
        return selectMeasuredVelocityAccordingMotionConsistency(candidates);
    }
    // Select measured velocity among candidates according motion consistency 与历史速度预估最接近的值作为速度观测值
    ///@note choose candidate with least measured_velocity_gain
    Eigen::Vector3f RobustKalmanFilter::selectMeasuredVelocityAccordingMotionConsistency(const std::vector<Eigen::Vector3f>& candidates)
    {
     ...
     * @return
     */
    Eigen::Vector3f RobustKalmanFilter::calcMeasuredAcceleration(const Eigen::Vector3f& measured_velocity,
                                                                 const double& time_diff)
    {
     ...
     */
    void RobustKalmanFilter::execVelocityUpdate(const Eigen::VectorXf& measured_anchor_point,
    ...
    // Set breakdown threshold as 2 times of velocity covariance, and smooth it somehow.
    ...
    /**
     * @brief 融合更新质量/故障阈值控制更新增益过度估计的加速度更新 ==> 通过加速度估计更新速度估计
     * @param measured_acceleration
     * @result belief_acceleration_
     */
    void RobustKalmanFilter::execAccelerationUpdate(const Eigen::VectorXf& measured_acceleration)
    {
        // Compute kalman gain
        Eigen::Matrix3d mat_c = Eigen::Matrix3d::Identity();
        ...
        Eigen::Vector3d acceleration_gain = mat_k * (measured_acceleration_d - mat_c * belief_acceleration_);
        // Adaptive
    ...
    void RobustKalmanFilter::evaluateOnlineCovariance()
    {
        ...
        int evaluate_window_size = history_measured_velocity_.size() > s_measurement_cached_history_size_maximum_ ?
        ...
        for (int i = 0; i < evaluate_window_size; ++i) {
            ...
            if (history_index >= 0) {
                velocity_residual = history_measured_velocity_[history_index].cast<double>() - belief_velocity_;
            }
    ...
    void RobustKalmanFilter::getAccelerationGain(Eigen::Vector3f* acceleration_gain)
    {
        (*acceleration_gain) = belief_acceleration_gain_.cast<float>();
    }

    void RobustKalmanFilter::getOnlineCovariance(Eigen::Matrix3d* online_covariance)
    {
